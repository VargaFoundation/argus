...
/*
 * Integration tests: Connect to a real Spark Thrift Server (STS).
 *
 * The Spark Thrift Server implements the HiveServer2 Thrift protocol, so it is
 * reached through the Hive backend (BACKEND=hive) — no Spark-specific backend
 * is required.
 *
 * Requires: docker compose -f tests/integration/docker-compose.yml up -d spark-thrift
 * Set SPARK_HOST and SPARK_PORT to override defaults (localhost:10010).
 */
...
static void test_alloc_stmt(void **state)
{
    ...
    char conn_str[512];
    snprintf(conn_str, sizeof(conn_str),
             "BACKEND=hive;HOST=%s;PORT=%d;UID=spark;AuthMech=NOSASL",
             get_spark_host(), get_spark_port());
...
int main(void)
{
    ...
    return cmocka_run_group_tests_name("spark_connect", tests, NULL, NULL);
}
